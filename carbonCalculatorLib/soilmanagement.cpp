...
double SoilManagement::computeEmissions(double carbonInSoil)
{
    double emissions;
    ...
    emissions += FROM_C_TO_CO2 * carbonInSoil * (percentageEmissions+0.001)*percentage.permanentGrass*0.01;
    emissions += FROM_C_TO_CO2 * carbonInSoil * (percentageEmissions+0.002)*percentage.coverCropping*0.01;
    emissions += FROM_C_TO_CO2 * carbonInSoil * (percentageEmissions+0.0025)*percentage.noTillage*0.01*(percentage.arable-percentage.coverCropping)*0.01;
    emissions += FROM_C_TO_CO2 * carbonInSoil * (percentageEmissions+0.003)*percentage.minimumTillage*0.01*(percentage.arable-percentage.coverCropping)*0.01;
    emissions += FROM_C_TO_CO2 * carbonInSoil * (percentageEmissions+0.004)*percentage.conventionalTillage*0.01*(percentage.arable-percentage.coverCropping)*0.01;
...
double SoilManagement::computeSequestrationRootBiomass()
{
    ...
    double biomassRootPerHectare = 625.25;
    ...
    biomassRootCrop = - biomassRootPerHectare * FROM_C_TO_CO2*exp(-1.27)*(100 - percentage.forest)*0.01;
    //if (!isOrganic) biomassRootPerHectare = 337.5;
    //else biomassRootPerHectare = 571.5;
    biomassRootPerHectare = 337.5;
    biomassRootCoverCrop = - biomassRootPerHectare * FROM_C_TO_CO2*exp(-1.27)*percentage.coverCropping*0.01*0.5;
    biomassRootPermanentGrass = - biomassRootPerHectare * FROM_C_TO_CO2*exp(-1.27)*percentage.permanentGrass*0.01;
    biomassRootForest = - 981 * FROM_C_TO_CO2*exp(-1.27)*percentage.forest*0.01;
    return biomassRootCrop + biomassRootCoverCrop + biomassRootPermanentGrass + biomassRootForest;
}

double SoilManagement::computeSequestrationUnstableCarbonDueToRoots()
{
    ...
    biomassRootPerHectare = 337.5;
    biomassRootCrop = biomassRootPerHectare*(100 - percentage.forest)*0.01*(exp(-rootDecayParameter));//*0.01*(percentage.conventionalTillage + percentage.noTillage * ((soilTillage[myIdClimate].matrix[2][0]-1)/20 + 1) + percentage.minimumTillage * ((soilTillage[myIdClimate].matrix[1][0]-1)/20 +1));
...
void SoilManagement::computeSequestration(double carbonInSoil, int myIdClimate, double* quantityOfAmendment, double* recalcitrantIndex, double* incrementalParameterAmendment, double* residues, bool* isIncorporatedResidue, double sequestrationFromRecalcitrantAmendment, double* dryMatter)
{
    ...
    double incrementUnstableRoots=1;
    double incrementUnstableResidues=1;
    ...
    incrementTotal *= computeSequestrationOrganicAmendments(2*computeSequestrationUnstableCarbonDueToRoots(),0.001545064,0);
    incrementUnstableRoots *= computeSequestrationOrganicAmendments(2*computeSequestrationUnstableCarbonDueToRoots(),0.001545064,0);
    incrementTotal *= 1;
    incrementUnstableRoots *= 1;
    ...
    for (int i=0;i<4;i++)
    {
        if (isIncorporatedResidue[i])
        {
            ...
            incrementUnstableResidues *= computeSequestrationOrganicAmendments(2*residues[i]*percentageUnstable[i]*1000,0.001545064,0);
        }
    }
    ...
    double sequesteredCarbonCO2EqCopy;
    sequesteredCarbonCO2EqCopy = sequestrationCarbonCO2Eq;
    double weightRoot,weightResidue,weightAmendment,weightTillage,weightLandUse,weightCoverCrop;
    weightRoot = weightResidue = weightAmendment = weightTillage = weightLandUse = weightCoverCrop = 0;
    double denominator = incrementTotal - 1;
    double sumWeights;
    /*sumOfIncrements = incrementUnstableRoots + incrementUnstableResidues
            + incrementOrganicAmendment + incrementCoverCrop
            + incrementLandUse + incrementTillage - 5;
    if (denominator > 0)
        denominator /= sumOfIncrements;*/
    if (denominator > 0)
    {
        weightRoot = (incrementUnstableRoots-1)/ denominator;
        weightResidue = (incrementUnstableResidues-1)/ denominator;
        weightAmendment = (incrementOrganicAmendment-1)/ denominator;
        weightCoverCrop = (incrementCoverCrop-1)/ denominator;
        weightLandUse = (incrementLandUse-1)/ denominator;
        weightTillage = (incrementTillage-1)/ denominator;
        sumWeights = weightAmendment + weightCoverCrop
                + weightLandUse + weightResidue
                + weightRoot + weightTillage;
        weightRoot /= sumWeights;
        weightResidue /= sumWeights;
        weightAmendment/= sumWeights;
        weightCoverCrop/= sumWeights;
        weightLandUse/= sumWeights;
        weightTillage/= sumWeights;
        if (weightRoot < 0) weightRoot = 0;
        if (weightResidue < 0) weightResidue = 0;
        if (weightAmendment < 0) weightAmendment = 0;
        if (weightCoverCrop < 0) weightCoverCrop = 0;
        if (weightLandUse < 0) weightLandUse = 0;
        if (weightTillage < 0) weightTillage = 0;
    }


    sequestrationCarbonCO2EqTillage = weightTillage * sequesteredCarbonCO2EqCopy;
    ...
    sequestrationCarbonCO2EqResidue[0] = SoilManagement::computeSequestrationResidueIncorporation2(residues[0],isIncorporatedResidue[0],0)* FROM_C_TO_CO2;
    sequestrationCarbonCO2EqResidue[1] = SoilManagement::computeSequestrationResidueIncorporation2(residues[1],isIncorporatedResidue[1],0)* FROM_C_TO_CO2;
    sequestrationCarbonCO2EqResidue[2] = SoilManagement::computeSequestrationResidueIncorporation2(residues[2],isIncorporatedResidue[2],1)* FROM_C_TO_CO2;
    sequestrationCarbonCO2EqResidue[3] = SoilManagement::computeSequestrationResidueIncorporation2(residues[3],isIncorporatedResidue[3],1)* FROM_C_TO_CO2;
    ...
    sequestrationCarbonCO2EqResidueGreen += 0.5*weightResidue*sequesteredCarbonCO2EqCopy;
    sequestrationCarbonCO2EqResidueWood += 0.5*weightResidue*sequesteredCarbonCO2EqCopy;
    sequestrationCarbonCO2Eq += SoilManagement::computeSequestrationResidueIncorporation2(residues[0],isIncorporatedResidue[0],0)* FROM_C_TO_CO2;
    sequestrationCarbonCO2Eq += SoilManagement::computeSequestrationResidueIncorporation2(residues[1],isIncorporatedResidue[1],0)* FROM_C_TO_CO2;
    sequestrationCarbonCO2Eq += SoilManagement::computeSequestrationResidueIncorporation2(residues[2],isIncorporatedResidue[2],1)* FROM_C_TO_CO2;
    sequestrationCarbonCO2Eq += SoilManagement::computeSequestrationResidueIncorporation2(residues[3],isIncorporatedResidue[3],1)* FROM_C_TO_CO2;
...
double SoilManagement::computeSequestrationResidueIncorporation2(double residueIncorporated, bool isIncorporatedResidue,int isHerbaceous)
{
